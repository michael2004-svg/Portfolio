...
const heading = "font-display text-2xl text-ice md:text-3xl";
...
function OptionCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "w-full rounded-2xl glass px-5 py-4 text-left font-body text-sm transition-all",
        selected
          ? "border-violet-300 bg-violet-500/10 text-ice ring-1 ring-violet-300/50"
      ...
      <div className="grid grid-cols-2 gap-3">
        {DEADLINE_OPTIONS.map((d) => (
          <OptionCard
            key={d.id}
            label={d.label}
            selected={data.deadline === d.id}
            onClick={() => update({ deadline: d.id })}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <div className={fieldShell}>
      ...
      <textarea
        ...
        className="w-full resize-none rounded-2xl glass px-5 py-4 font-body text-sm text-ice placeholder:text-muted/60 focus:outline-none focus:ring-1 focus:ring-violet-300/50"
      />
    </div>
  );
}
...
export function StepUploads({
  ...
  return (
    <div className={fieldShell}>
      ...
      <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl glass px-5 py-10 text-center transition-colors hover:border-violet-300/50">
        <span className="font-mono text-xs uppercase tracking-wide text-violet-300">
          Click to upload
        </span>
      ...
      {data.fileNames.length > 0 && (
        <ul className="space-y-1">
          {data.fileNames.map((name) => (
            <li key={name} className="font-mono text-xs text-muted">
              · {name}
            </li>
          ))}
        </ul>
    ...
    <div className={fieldShell}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="mt-2 font-display text-2xl text-ice">
            {estimate.complexity}
          </div>
        </motion.div>
        <motion.div
        ...
        <motion.div
          ...
          className="col-span-2 rounded-2xl glass p-5"
        >
          <span className="font-mono text-[10px] uppercase tracking-widest text-violet-300">
            Suggested budget
          </span>
          <div className="mt-2 font-display text-2xl text-ice">
            {estimate.suggestedBudget}
          </div>
        </motion.div>
      </div>
